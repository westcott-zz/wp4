class Ticket < ActiveRecord::Base
  attr_accessible :description, :title, :status, :due_at, :completed_at, :category, :user_id, :asset_id, :asset_type, :bucket, :assigned_to
  belongs_to :user
  belongs_to :contact
...
#  serialize :subscribed_users, Set
  ...
  # Tickets created by the user for herself, or assigned to her by others. That's
  # what gets shown on Tickets/Pending and Tickets/Completed pages.
  scope :my, lambda { |*args|
    ...
    order(options[:order] || 'title ASC').
    limit(options[:limit]) # nil selects all records
  }

  # Tickets assigned by the user to others. That's what we see on Tickets/Assigned.
  scope :assigned_by, lambda { |user|
    includes(:assignee).
    where('user_id = ? AND assigned_to IS NOT NULL AND assigned_to != ?', user.id, user.id)
  }

  # Tickets created by the user or assigned to the user, i.e. the union of the two
  # scopes above. That's the tickets the user is allowed to see and track.
  ...
  scope :pending,       where('completed_at IS NULL').order('tickets.due_at, tickets.id')
  scope :assigned,      where('completed_at IS NULL AND assigned_to IS NOT NULL').order('tickets.due_at, tickets.id')
  scope :completed,     where('completed_at IS NOT NULL').order('tickets.completed_at DESC')

  # Due date scopes.
  scope :due_asap,      where("due_at IS NULL AND bucket = 'due_asap'").order('tickets.id DESC')
  scope :overdue,       where('due_at IS NOT NULL AND due_at < ?', Time.zone.now.midnight.utc).order('tickets.id DESC')
  scope :due_today,     where('due_at >= ? AND due_at < ?', Time.zone.now.midnight.utc, Time.zone.now.midnight.tomorrow.utc).order('tickets.id DESC')
  scope :due_tomorrow,  where('due_at >= ? AND due_at < ?', Time.zone.now.midnight.tomorrow.utc, Time.zone.now.midnight.tomorrow.utc + 1.day).order('tickets.id DESC')
  scope :due_this_week, where('due_at >= ? AND due_at < ?', Time.zone.now.midnight.tomorrow.utc + 1.day, Time.zone.now.next_week.utc).order('tickets.id DESC')
  scope :due_next_week, where('due_at >= ? AND due_at < ?', Time.zone.now.next_week.utc, Time.zone.now.next_week.end_of_week.utc + 1.day).order('tickets.id DESC')
  scope :due_later,     where("(due_at IS NULL AND bucket = 'due_later') OR due_at >= ?", Time.zone.now.next_week.end_of_week.utc + 1.day).order('tickets.id DESC')
  ...
  validates_presence_of :title, :message => :missing_ticket_title
  ...
  # Returns list of tickets grouping them by due date as required by tickets/index.
  #----------------------------------------------------------------------------
  def self.find_all_grouped(user, view)
    settings = (view == "completed" ? Setting.ticket_completed : Setting.ticket_bucket)
    Hash[
  ...
  # Returns ticket totals for each of the views as needed by tickets sidebar.
  #----------------------------------------------------------------------------
  ...
  ActiveSupport.run_load_hooks(:fat_free_crm_ticket, self)

end


# we will probably define ticket status here
#--hard coded since this will not change much...
# no need for a status table
